...
使用例:
    # プレビューのみ（TiDB への書き込みなし）
    uv run --env-file .env discord/verify_tidb_manual.py

    # TiDB への書き込みを実行する場合は --apply を付ける
    uv run --env-file .env discord/verify_tidb_manual.py --apply
"""
...
def main() -> None:
    ...
    print("=== TiDB 手動確認 ===")
    if not apply:
        print("（プレビューモード: TiDB への書き込みは行いません。書き込むには --apply を付けてください）")
    ...
    for channel_id in channel_ids:
        ...
        if apply:
            ...
            print("  （--apply なし: TiDB への書き込みをスキップ）\n")
